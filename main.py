import os, time, random, threading, json, re, logging, sqlite3, uuid, hashlib, html as htmlmod, csv, io, socket
...
def init_db():
    try:
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_tracking_id ON email_log(tracking_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_email ON email_log(email)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_status ON email_log(status)")
        conn.commit()
        conn.close()
...
def load_sheet_memory():
    ...
    url = get_cfg("APPS_SCRIPT_WEB_URL")
    if not url:
        push_log("No APPS_SCRIPT_WEB_URL set — sheet memory disabled")
    ...
    try:
        ...
        push_log(f"Sheet memory load failed: {e} — continuing without sheet dedup")
        with sheet_memory_lock:
            sheet_memory_loaded = True
...
def ai_gen_keywords(original: str, used: list) -> list:
    ...
    prompt = (
        ...
        f"Generate 8 NEW Play Store search keywords closely related to "
        f"'{original}' — synonyms, related terms, long-tail extensions, "
        f"and category variations. These keywords MUST find real, established apps "
        f"with good ratings and active user bases (trusted apps that need more reviews).\n\n"
        f"Example for 'meditation': meditation app, mindfulness, guided meditation, "
        f"meditation for sleep, meditation timer, calm, breathing exercise, meditation music\n\n"
        f"AVOID keywords for: calculator, converter, scanner, keyboard, launcher, "
        f"wallpaper, ringtone, flash, torch, weather, barcode, file manager, clock.\n\n"
    ...
    for lang, country in SEARCH_COMBOS:
        ...
        for item in results:
            ...
            if is_duplicate_in_sheet(app_id, ""):
                ...
                push_log(f"  Skip (in sheet): {app_id}")
                continue
            ...
            if email in global_seen_emails or is_duplicate_in_sheet("", email):
                global_seen_ids.add(app_id)
                push_log(f"  Skip (email dup): {email}")
                continue
...
def run_automation(initial_kw: str, target: int, hunter: dict = None):
    ...
    for i, lead in enumerate(all_leads):
        ...
        push_log(f"  Writing email for {lead['app_name']} ...")
...
def run_send_pending(leads: list):
    ...
    for i, lead in enumerate(leads):
        if stop_event.is_set():
            push_log("Stopped.")
            break
        push_log(f"  Writing email for {lead.get('app_name','')} ...")
        subject, body = ai_gen_email(lead, base_subject, base_body)
        ok = send_email_tracked(lead, subject, body, get_cfg("APP_URL", ""))
        if ok:
            sent += 1
...
def api_spam_test():
    ...
    if not test_to or not EMAIL_RE.match(test_to):
        return jsonify({"error": "valid test_email required"}), 400
    with cfg_lock:
        global run_cfg
        run_cfg = {
            "GROQ_API_KEY":     data.get("groq_key")         or os.environ.get("GROQ_API_KEY", ""),
    ...
    sample = {
        "app_name":   (data.get("sample_app_name") or "MyApp Pro")[:100],
        "developer":  (data.get("sample_developer") or "John Dev")[:100],
...
def api_analytics_email(tracking_id):
    if not tracking_id or len(tracking_id) > 100:
        return jsonify({"ok": False, "error": "Invalid tracking ID"}), 400
    conn = None
    try:
        conn = get_db()
